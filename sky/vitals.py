"""In-process rolling counters for public sky vitals (#408)."""
...
class SkyVitalsStore:
    ...
    def record_tool_event(self, event: ToolCallEvent) -> None:
        self.record_invocation(event.tool_name, timestamp=event.timestamp)
    ...
    def _prune_locked(self, now: float) -> None:
        ...
        self._invocations = deque(ts for ts in self._invocations if ts >= cutoff)
        self._resolves = deque(ts for ts in self._resolves if ts >= cutoff)
        self._seals = deque(ts for ts in self._seals if ts >= cutoff)
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        return {
            ...
            "activity": {
                "invocations_1h": sum(
                    1 for ts in invocations if ts >= now - _HOUR_SECONDS
                ),
