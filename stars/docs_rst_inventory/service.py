"""Deterministic RST/Sphinx tree inventory per ADR 0008 analyze-stage fields."""
...
from .contract import FEATURE_CLASSES, MAX_ENTRIES, MAX_ENTRY_BYTES, MAX_FINDINGS, MAX_MESSAGE_BYTES
from .parser import RawFinding, scan_document

BLOCKING_CLASSES: frozenset[str] = frozenset(
    {"decision_required", "unsupported", "malformed"}
)


def inventory(entries: Sequence[Mapping[str, Any]] | None = None) -> dict[str, object]:
    """Build a digest-bound inventory for a bounded RST documentation tree."""
    if entries is None:
    ...
    blocking = [
        {
            ...
            "class": item["class"],
            "path": item["path"],
            **({"message": item["message"]} if item.get("message") else {}),
        }
        for item in findings
        if item.get("class") in BLOCKING_CLASSES
    ]
    return {
        **inventory_body,
        ...
        "conversion_eligible": len(blocking) == 0,
        "ineligibility_reasons": blocking,
    }
