"""Pure static validation of one bounded public-source row shape."""
...
def validate(profile: str = DEFAULT_PROFILE, row: object = None) -> dict[str, object]:
    """Validate without fetching, storing, or echoing caller-provided values."""
    definition = PROFILES.get(profile)
    if definition is None:
        return {"error": "profile_not_allowed", "profile": profile, "live_at_call": True}
    fields = definition["fields"]
    errors = _errors(row, fields)
    ...
    if not errors:
        ...
        result["normalized_row"] = {field: row[field] for field in fields}
    return result


def _errors(row: object, fields: Mapping[str, object]) -> list[dict[str, str]]:
    ...
    for name, rule in fields.items():
        ...
        if rule["type"] == "string":
            if not isinstance(value, str):
                ...
                errors.append(
                    {"path": f"$.{name}", "code": "pattern", "expected": "uppercase IATA code"}
                )
