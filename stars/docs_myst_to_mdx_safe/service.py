"""Safe MyST → MDX plan/apply per ADR 0008 stage artifacts (#170)."""
...
from .transform import baseline_mdx_buildable, transform_document
...
def plan(
    ...
    """Build an ADR 0008 plan for the corpus-backed safe MyST→MDX subset."""
    ...
    planned_ops = _planned_ops(findings)
...
def apply(
    ...
    transform_paths = {
        ...
        and op.get("op") == "transform_admonition"
        and isinstance(op.get("path"), str)
    }
    malformed_paths = {
        str(item["path"])
        for item in findings
        if item.get("class") == "malformed" and isinstance(item.get("path"), str)
    }
    ...
    for entry in normalized:
        ...
        if path in malformed_paths:
            # Never "repair" malformed fences into MDX.
            target = source
        elif path in transform_paths:
            # Transform corpus admonitions; leave unsupported directives intact.
            target = transform_document(source, allow_admonition=True)
        else:
            # hold / copy_heading-only: identity bytes preserve mapping.
            target = source
    ...
    validation = baseline_mdx_buildable(safe_targets)
...
def _planned_ops(findings: Sequence[Mapping[str, Any]]) -> list[dict[str, object]]:
    ...
    for item in findings:
        ...
        if feature_id in CORPUS_SAFE_FEATURES:
            key = ("copy_heading", path, feature_id)
            if key not in seen:
                seen.add(key)
                ops.append(
                    {
        ...
        if feature_id in CORPUS_TRANSFORMABLE_FEATURES and class_ == "transformable":
            key = ("transform_admonition", path, feature_id)
            if key not in seen:
                seen.add(key)
                ops.append(
                    {
                        "op": "transform_admonition",
                        "path": path,
                        "feature_id": feature_id,
                    }
                )
        ...
        # Non-corpus roles / directives outside this star's named subset — hold.
