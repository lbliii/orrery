"""API-spec upgrade resumable constellation (ADR 0007/0008, #179).

Frozen stage graph: inventory → choose-profile → safe-upgrade →
breaking-approval (pause when needed) → validate-target → compatibility-diff →
artifact-seal. Consumes sealed shapes from migration stars — does not reimplement.
"""
...
from stars.api_spec_compatibility_diff.service import compatibility_diff
from stars.api_spec_openapi_inventory.service import inventory as build_inventory
from stars.api_spec_openapi_upgrade_safe.service import apply as safe_apply
from stars.api_spec_openapi_upgrade_safe.service import plan as safe_plan
from stars.api_spec_openapi_validate.service import validate as validate_target
from stars.decision_bind.service import bind as bind_decision

CONSTELLATION = "orrery/api-spec-upgrade"
DISPOSITIONS = (
    "completed",
    "awaiting_input",
    "inconclusive",
...
ACTION_KIND = "breaking_change_approval"
DECISION_ACTIONS = frozenset({"approve", "abort"})
PAUSE_CLASSES = frozenset({"decision_required", "unsupported", "malformed"})
...
_COMPONENTS = (
    {"name": "orrery/api-spec-openapi-inventory", "version": "0.1.0"},
    {"name": "orrery/api-spec-openapi-upgrade-safe", "version": "0.1.0"},
    {"name": "orrery/api-spec-openapi-validate", "version": "0.1.0"},
    {"name": "orrery/api-spec-compatibility-diff", "version": "0.1.0"},
)
_LIMITATIONS = (
    "Frozen OpenAPI upgrade graph — inventory/upgrade/validate/diff stars consumed.",
    "Pause only for breaking/unknown/decision-required inventory findings.",
    ...
    "validate-target and compatibility-diff digests remain distinct in receipt.",
...
def run(
    ...
    *,
    ...
    skill_name: str = "api-spec-upgrade",
    skill_version: str = "0.1.0",
    key_id: str = "orrery-api-spec-upgrade-1",
    private_key: Any = None,
) -> dict[str, Any]:
    """Start an api-spec/upgrade run through inventory, profile pin, and safe upgrade."""
    ...
    gate_result, gate_error = _run_through_safe_upgrade(normalized, pinned)
    ...
    approval_findings = _approval_required_findings(gate_result["findings"])
    if not approval_findings:
    ...
    required_ids = sorted({str(item["feature_id"]) for item in approval_findings})
    action_request = ActionRequest(
        ...
        title="Breaking or unknown OpenAPI constructs require approval",
        prompt=(
            "Choose approve (proceed with policy exception) or abort for each "
            f"finding: {', '.join(required_ids)}."
        ),
    )
...
def continue_run(
    ...
    *,
    ...
    """Resume a paused upgrade run and seal the composite receipt (idempotent)."""
...
def cancel(run_id: str, *, caller_id: str = "anonymous") -> dict[str, Any]:
    """Cancel a paused or in-flight upgrade run."""
    return cancel_checkpoint(run_id, caller_id=caller_id)


def _run_through_safe_upgrade(
    ...
    stage_digests = (
        ...
        stage_receipt_digest(upgrade_stage),
    )
...
def _seal_terminal(
    *,
    ...
    diff_result = compatibility_diff(
        source_entries=entries,
        target_entries=targets,
    ...
    diff_stage = {
        "stage": "compatibility-diff",
        "diff_digest": diff_result["diff_digest"],
        "policy_id": diff_result["policy_id"],
        "change_count": diff_result["change_count"],
        "runtime_compatibility_claimed": diff_result["runtime_compatibility_claimed"],
    }
    ...
    if decisions:
        ...
        for item in decisions:
            ...
            bound = bind_decision(
                f"openapi-{feature_id}",
                (
                    f"Policy exception: action={action} for "
                    f"{feature_id}; compatibility-diff digests unchanged."
                ),
            )
            if "error" in bound:
                return dict(bound)
    ...
    all_stage_digests.append(stage_receipt_digest(diff_stage))
    seal_stage = {
        "stage": "artifact-seal",
        "receipt_digest": migration_receipt["receipt_digest"],
        "output_manifest_digest": output_manifest_digest,
        "compatibility_diff_digest": diff_result["diff_digest"],
    }
    ...
    chain_steps = _chain_steps(
        ...
        validation_stage_digest=all_stage_digests[-3],
        diff_stage_digest=all_stage_digests[-2],
    ...
    composite: dict[str, Any] = {
        ...
        "transformer": dict(profile["transformer"]),
        "validator": dict(profile["validator"]),
        "compatibility_policy": {
            "policy_id": profile["compatibility_policy"]["policy_id"],
        },
        "bundle_digest": apply_result["bundle_digest"],
        "validation_digest": validation_result["validation_digest"],
        "compatibility_diff_digest": diff_result["diff_digest"],
    ...
    if cites:
        ...
        composite["stages"]["breaking-approval"] = {
...
def _chain_steps(
    *,
    ...
    diff_stage_digest: str,
    seal_stage_digest: str,
    seal_envelope: dict[str, Any],
    paused: bool,
) -> list[dict[str, Any]]:
    labels = ["inventory", "choose-profile", "safe-upgrade"]
        ...
        steps.append(
            {
                ...
                "label": "breaking-approval",
                "status": "skipped",
                "note": "no breaking/unknown/decision-required findings",
            }
        )
        order += 1
    steps.extend(
        [
            ...
            },
            {
                ...
                "status": "Envelope ✓",
                "stage_receipt_digest": diff_stage_digest,
            },
            {
                "order": order + 2,
                "label": "artifact-seal",
...
def _approval_required_findings(
    findings: Sequence[Mapping[str, Any]],
) -> list[dict[str, Any]]:
    return [
        dict(item)
        for item in findings
        if item.get("class") in PAUSE_CLASSES
    ]
