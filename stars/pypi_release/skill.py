...
def build_skill(*, private_key: Any | None = None) -> Skill:
    private, key_id = public_star_signing_key(
        ...
        private_key_env="ORRERY_PYPI_RELEASE_PRIVATE_KEY",
        key_id_env="ORRERY_PYPI_RELEASE_KEY_ID",
        default_key_id="orrery-pypi-release-1",
    )
    skill = Skill(
        "pypi-release",
    ...
    @skill.tool("get", description="Get current metadata for a named allowlisted PyPI package")
