/* Vendored htmx-ext-sse@2.2.2 (BSD). Chirp htmx 2 inject is core-only;
   the home feed still needs hx-ext="sse". Wait for Chirp-injected htmx
   when this file is parsed first (layout <head> vs inject before </body>). */
(function waitForHtmx() {
  if (typeof htmx === "undefined") {
    document.addEventListener("DOMContentLoaded", waitForHtmx, { once: true });
    setTimeout(waitForHtmx, 0);
    return;
  }
  if (window.__orreryHtmxSse) return;
  window.__orreryHtmxSse = true;
/*
