...
from stars.csv_url.contract import MAX_ROWS, tool_schemas
from stars.csv_url.service import get
from stars.csv_url.skill import build_skill

URL = "https://raw.githubusercontent.com/vega/vega-datasets/main/data/cars.csv"
SOURCE = b"name,year,mpg,electric\nPinto,1971,25.0,false\nLeaf,2019,,true\n"
...
def test_named_dataset_uses_exact_canonical_source_and_typed_bounded_rows() -> None:
    ...
    assert result["rows"] == [
        {"name": "Pinto", "year": 1971, "mpg": 25.0, "electric": False},
        {"name": "Leaf", "year": 2019, "mpg": None, "electric": True},
    ]
...
def test_unknown_dataset_never_fetches_and_malformed_csv_is_loud() -> None:
    ...
    assert (
        get("cars", fetch=lambda url, **_: (url, 200, {}, malformed))["error"] == "source_malformed"
    )
    uneven = b"name,year\nPinto,1971,unexpected\n"
    assert get("cars", fetch=lambda url, **_: (url, 200, {}, uneven))["error"] == "source_malformed"


def test_rejects_final_source_escape_and_truncates_rows() -> None:
    source = b"value\n" + b"\n".join(str(number).encode() for number in range(MAX_ROWS + 1))
    result = get("cars", fetch=lambda url, **_: (url, 200, {}, source))
    ...
    assert (
        get(
            "cars",
            fetch=lambda _url, **_: (
                "https://raw.githubusercontent.com/vega/other/main/data/cars.csv",
                200,
                {},
                SOURCE,
            ),
